...
■繰り返し出費にはは複合キーを使う？
複合キーを持たせて、updateしたら過去のレコードに複合キーをインクリメントする。
まずはidをprimary_keyでなくする↓
https://railsguides.jp/active_record_migrations.html
>デフォルトでは、create_tableによってidという名前の主キーが作成されます。:primary_keyオプションを指定することで、主キー名を変更することもできます (その場合は必ず対応するモデル名を変更してください)。 主キーを使用したくない場合はid: falseオプションを指定することもできます。

そして、sub_idカラムを追加してidとsub_idでindexとuniqueをはって複合主キーを作る？↓↓
https://qiita.com/qsona/items/738be3c9f69d0818944f
そうすると、繰り返し出費をupdateするとsub_idがインクリメントされて、未来のレコードはそれを読み込むようにする。どっちにしろ、何を変更したかで処理を変える必要がありそう。

■bought_buttonsは何回も押してしまいがちだから、とりあえずactive jobでjobテーブルに入れて、10分後とかに変わっていれば表示する。
...
レコードは簡単に消去しないほうがいい時がある。
退会したあとに、また戻ってきたときとかに、前の状態をすぐに復元できるととてもいい。
出費で言うと間違って消去したものを再度入力しなくても復元できると、楽である。
一人の場合はそんなに意味はないがパートナーと使っていると、
消去した情報は見えたほうがいい。
そこで三つの方法がある。
▽一つは、まったく同じテーブルを二つ作り、どちらを見るかを紐づけるテーブルもつくる。
例えば、
・expenses table
  | id | user_id | show | deleted |
  消去されるまではshowにexpenses_att_idを。消去されればdeleted_expneses_attにレコードをそっくりそのままコピーして、expenses_attのレコードを消去する。そしてdeltedカラムにdeleted_expenses_att_idを入れる。戻すときはそれを逆のことをする。  
・expenses_att table  
・deleted_expesnes_att table

▽もう一つは単純にフラグをつける方法
t.boolean :deleted_flg, default: false
消去された出費の画面では、deleted_flgがtrueになってるものを引いてくる。

▽何でもかんでも入れられる。deleted_records tableを作る。

■notification model
  キューに入っているjobの種類を調べて、bought_buttonsControllerからのjobがあれば、キューに入れない処理にすれば、10分後にまとめて処理できそう。。
  Jobに入れるcontrollerでdelayed_jobのjob_classを参照できたら、できそう。
  ajaxみたいにすぐ反映されものにはうまく行くかも。
  ▽bought_button_controller
    ajax後キューにjobを入れるけど、入れる前に先に同じようなjobがあるか調査。なければ、
    return if 条件
    でキューには入れない。調査部分はキューに入れるメソッドと分ける。
  ▽Notification model
    最終的に通知するレコードを生成する処理を書く。
  ▽notify_bought_jobでは
    Notification.rbで作成したメソッドを実行する処理を記載

  ※delayed_jobsテーブルにuser_idがすぐに参照できない
  →delayed_jobs.all.where(created_at > @want.updated_at)のものを取り出して、それぞれの
  handllerの値を.mapして、JSON.parseして['job_data']['job_class']に実行クラス名を引いてこれる。さらにargumentからuser_idなるものを引いてこれればできるが、処理がめちゃかかりそう。とりあえず試してみて、遅すぎたらやめよう。ただ、全てのnotificationが実装できてから試す。大規模になると間違いなく無理だけど、大規模だとcronとか使えるから。ajaxなのにめちゃ遅くなるから違う方法がいいかもしれない。
  変更するコード
